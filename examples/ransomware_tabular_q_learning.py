...
if __name__ == '__main__':
    ...
    q_agent_config = QAgentConfig(gamma=0.999, alpha=0.0005, epsilon=1, render=False, eval_sleep=0.9,
                                  ...
                                  video_fps=5, video_dir=os.path.join(default_output_dir(), "results", "videos", str(random_seed)), num_episodes=6001,
                                  eval_render=False, gifs=True, gif_dir=os.path.join(default_output_dir(), "results", "gifs", str(random_seed)),
                                  eval_frequency=300, attacker=True, defender=False, video_frequency=101,
                                  save_dir=os.path.join(default_output_dir(), "results", "data", str(random_seed)))
    env_name = "ransomgame-minimal_defense-v0"
    env = gym.make(env_name, save_dir=os.path.join(default_output_dir(), "results", "data", str(random_seed)))
