"""
The viewer for rendering the gym-ransomgame environment. Supports both agent-mode and manual-mode
"""
try:
    import pyglet
...
import numpy as np
import time
from gym_ransomgame.envs.rendering.frames.game_frame import GameFrame
...
class Viewer():
    ...
    def manual_start_attacker(self) -> None:
        ...
        :return: None
        """
        self.ransomgame_config.game_config.manual_attacker = True
        self.gameframe = GameFrame(ransomgame_config=self.ransomgame_config)
    ...
    def manual_start_defender(self) -> None:
        ...
        :return: None
        """
        self.ransomgame_config.game_config.manual_defender = True
        self.gameframe = GameFrame(ransomgame_config=self.ransomgame_config)
        self.gameframe.on_close = self.window_closed_by_user
        self.isopen = True
        pyglet.clock.schedule_interval(self.gameframe.update, 1 / 60.)
    ...
    def agent_start(self) -> None:
        ...
        """
        self.ransomgame_config.game_config.manual_attacker = False
        self.ransomgame_config.game_config.manual_defender = False
        self.gameframe = GameFrame(ransomgame_config=self.ransomgame_config)
        self.gameframe.on_close = self.window_closed_by_user
        self.isopen = True
    ...
    def render_frame(self, return_rgb_array: bool = False):
        ...
        return self.extract_rgb_array() if return_rgb_array else self.isopen
    ...
    def render(self, return_rgb_array = False):
        ...
        frames.append(arr)
        for i in range(self.gameframe.ransomgame_config.render_config.num_blinks):
            if len(self.gameframe.game_state.defense_events) > 0 or len(self.gameframe.game_state.attack_events) > 0:
                ...
                frames.append(arr)
                time.sleep(self.gameframe.ransomgame_config.render_config.blink_interval)
