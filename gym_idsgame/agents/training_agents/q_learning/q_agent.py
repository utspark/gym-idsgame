...
class QAgent(TrainAgent, ABC):
    ...
    def __init__(self, env: Any, config: QAgentConfig):
        ...
        self.num_train_ransomware_games_total = 0
        ...
        self.outer_train = tqdm.tqdm(
            total=self.config.num_episodes, desc="Train Episode", position=0
        )
    ...
    def log_metrics(
        ...
        hack_probability = (
            self.train_hack_probability if not eval else self.eval_hack_probability
        )
        hack_probability_total = (
            self.train_cumulative_hack_probability
            if not eval
            else self.eval_cumulative_hack_probability
        )
        ...
        if eval:
            log_str = (
                "[Eval] episode: {:>5}, avg_a_R: {:.2f}, avg_d_R: {:.2f}, avg_t: {:.2f}, avg_h: {:.2f}, "
                "acc_A_R: {:.2f}, acc_D_R: {:.2f}, replay_s: {}, lr: {:.2E}, c_h: {:.2f}".format(
                    episode,
                    avg_attacker_episode_rewards,
            ...
            log_str = (
                "[Train] episode: {:>5}, epsilon: {:.2f}, avg_a_R: {:.2f}, avg_d_R: {:.2f}, avg_t: {:.2f}, "
                "avg_h: {:.2f}, acc_A_R: {:.2f}, acc_D_R: {:.2f}, A_loss: {:.6f}, D_loss: {:.6f}, "
                "replay_s: {}, lr: {:.2E}, c_h: {:.2f}".format(
                    episode,
                    self.config.epsilon,
                    avg_attacker_episode_rewards,
                    avg_defender_episode_rewards,
        ...
        if (
            update_stats
            and self.config.dqn_config is not None
            and self.config.dqn_config.tensorboard
        ):
    ...
    def log_tensorboard(
        self,
        episode: int,
        avg_attacker_episode_rewards: float,
        avg_defender_episode_rewards: float,
        avg_episode_steps: float,
        episode_avg_loss_attacker: float,
        episode_avg_loss_defender: float,
        hack_probability: float,
        attacker_cumulative_reward: int,
        defender_cumulative_reward: int,
        epsilon: float,
        lr: float,
        cumulative_hack_probability: float,
        eval=False,
    ) -> None:
        """
        ...
        self.tensorboard_writer.add_scalar(
            "avg_episode_rewards/" + train_or_eval + "/attacker",
            avg_attacker_episode_rewards,
            episode,
        )
        self.tensorboard_writer.add_scalar(
            "avg_episode_rewards/" + train_or_eval + "/defender",
            avg_defender_episode_rewards,
            episode,
        )
        self.tensorboard_writer.add_scalar(
            "episode_steps/" + train_or_eval, avg_episode_steps, episode
        )
        self.tensorboard_writer.add_scalar(
            "episode_avg_loss/" + train_or_eval + "/attacker",
            episode_avg_loss_attacker,
            episode,
        )
        self.tensorboard_writer.add_scalar(
            "episode_avg_loss/" + train_or_eval + "/defender",
            episode_avg_loss_defender,
            episode,
        )
        self.tensorboard_writer.add_scalar(
            "hack_probability/" + train_or_eval, hack_probability, episode
        )
        self.tensorboard_writer.add_scalar(
            "cumulative_hack_probability/" + train_or_eval,
            cumulative_hack_probability,
            episode,
        )
        self.tensorboard_writer.add_scalar(
            "cumulative_reward/attacker/" + train_or_eval,
            attacker_cumulative_reward,
            episode,
        )
        self.tensorboard_writer.add_scalar(
            "cumulative_reward/defender/" + train_or_eval,
            defender_cumulative_reward,
            episode,
        )
